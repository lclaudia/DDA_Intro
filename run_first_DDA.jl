...
for n_LIST = 1:size(LIST, 1)
    ...
    for wn = 0:WN-1
        ...
        data1 = Y[anf+1:ende+1, ch1]
        ddata1 = deriv_all(data1, dm)
        data1 = data1[dm+1:end-dm]
        STD = std(data1)
        ...
        data2 = Y[anf+1:ende+1, ch2]
        ddata2 = deriv_all(data2, dm)
        data2 = data2[dm+1:end-dm]
        STD = std(data2)
...
ST2 = readdlm("ROS_4.DDA_ST");
ST2 = ST2[:, 3:end];
CT2 = readdlm("ROS_4.DDA_CT");
CT2 = CT2[:, 3:end];

println(mean(reshape(ST, WN, size(ST, 2) * size(ST, 3)) .- ST2))
println(mean(reshape(CT, WN, size(CT, 2) * size(CT, 3)) .- CT2))
